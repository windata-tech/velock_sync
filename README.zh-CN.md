...
Velock Sync 是一款开源、跨平台的增量同步工具，承担两类任务：为格间（Velock）提供持续的零知识远程备份，以及为用户选择的数据提供通用双向文件夹同步。格间数据只会以已经加密并认证的形式传输，Sync 不接触格间明文和密钥。
...
- 通用文件夹同步：选择一个本地文件夹和 WebDAV、Google Drive 或 OneDrive 目标，在多设备间进行冲突安全的增量双向同步。
